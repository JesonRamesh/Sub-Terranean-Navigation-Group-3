function [X_Est, P_Est, GT] = EKF4(out)
    %% 1. Data Extraction & Ground Truth Initialization
    ...
    Q = diag([5e-5, 5e-5, 1e-7, 0.3, 0.3]); % Slightly increased process noise
    R_tof = 0.003;   % Reverting to tighter ToF noise
    R_mag = 0.4;     % Slightly increased magnetometer noise

    gate_threshold = 1.5; % Relaxed gating threshold
    gate_abs = 0.35;      % Relaxed absolute gating cap

    vel_decay = 0.975;    % Faster velocity decay to reduce drift
