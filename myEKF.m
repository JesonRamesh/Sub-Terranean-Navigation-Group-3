function [X_Est, P_Est, GT] = myEKF(out)
    ...
    gyro_bias = mean(gyro_data(1:50, :));
    accel_bias = mean(accel_data(1:50, :));
    ...
    % gt_rot_data = squeeze(out.GT_rotation.signals.values)';
    % gt_quat0 = quaternion(gt_rot_data(1, :));
    % eul0 = euler(gt_quat0, 'ZYX', 'frame'); 
    % theta0 = eul0(1);

    % raw_quat = out.GT_rotation.signals.values(:, :, 1);
    % q = raw_quat(:)'; 
    % 
    ...
    dy = gt_pos(end, 2) - gt_pos(1, 2);
    dx = gt_pos(end, 1) - gt_pos(1, 1);
    theta0 = atan2(dy, dx);

    % Calculate initial magnetometer reading
    mag_x0 = (mag_data(1, 1) - mag_hard_iron(1)) * mag_soft_iron(1);
    mag_y0 = (mag_data(1, 2) - mag_hard_iron(2)) * mag_soft_iron(2);
    z_mag0 = atan2(mag_y0, mag_x0);
    
    % Align Magnetic North with PhaseSpace Arena X-axis
